...
Scenario: Testing the GET method for session-recall
  Given url rapidtox + '/rapidtox/bioactivity/toxcast/by-dtxsids'
...
Scenario: Testing a GET endpoint with DTXSID as a search parameter
  Given url rapidtox + '/rapidtox/toxcast-model/search/by-dtxsid?dtxsid=DTXSID001001267'
  When method GET
  Then status 200
  And response.dtxsid == 'DTXSID001001267'

Scenario: Testing a GET endpoint with an invalid DTXSID
  Given url rapidtox + '/rapidtox/toxcast-model/search/by-dtxsid?dtxsid='
  When method GET
  Then status 200
